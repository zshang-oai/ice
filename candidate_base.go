...
func (c *candidateBase) handleCandidatePairPacket(buf []byte, srcAddr net.Addr) {
	agent := c.agent()
	selected := agent.getSelectedPair()
	// Keep ordinary selected-path traffic off the task loop.
	if selected == nil || selected.Local.Equal(c) && addrEqual(selected.Remote.addr(), srcAddr) {
		return
	}
	...
	if err := agent.loop.Run(c, func(_ context.Context) {
		...
		if agent.userCandidatePairPacketHandler(buf, pair, selected) {
			if agent.lite {
				// The application-approved lite path enters the valid list
				// without an outbound connectivity check.
